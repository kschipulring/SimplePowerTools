...
function load_template($path) {
    ...
    if ($pathinfo['extension'] == 'html') {
        return \SimplePowerTools\DOM_Query::loadHTML(file_get_contents($path));
    } else {
        return \SimplePowerTools\DOM_Query::loadHTML('<pre>' . htmlentities(file_get_contents($path)) . '</pre>');
    }
}
